...
This is V17 (V11's tapered evaluator with the hot per-square reduction jitted by numba, a
real 70.0% win over V11) with two additions carried over from earlier, rejected candidates:
V15's king-safety term (pawn shield, open files near the king, midgame only) and V16's
rook-on-open-file bonus. Neither cleared the bar on its own: V15 was a 45.0% wash against
V11, and re-tried alone on top of the faster V17 substrate as V18 it was still just a wash at
55.0% -- right at the other edge of the same band. V16 was a clear 37.5% regression against
V11.

This candidate tests whether combining the two cheap, still-plausible features (as opposed to
V16's rook bonus, which regressed even though it was cheap) does better than either alone, and
does so efficiently: both features need per-file pawn occupancy, so it is computed once per
side (a single pass over each side's pawns, at most 8 each) and shared between the two checks,
...
# King-safety weights: midgame only, same values as V15/V18.
KING_SHIELD_BONUS_MG = 10
KING_OPEN_FILE_PENALTY_MG = 20
# Rook-on-open-file bonuses: flat (both midgame and endgame), same values as V16.
...
# --- Tapered, PeSTO-style evaluation, same numbers as V11/V17/V18 -------------------------
#
...
def pawn_files(board: chess.Board) -> tuple[list[bool], list[bool]]:
    """Which files each side has at least one pawn on. Computed once per evaluation and
    shared between the king-safety and rook-on-open-file checks below, rather than paying
    for a separate pawn scan in each."""
    white_files = [False] * 8
    black_files = [False] * 8
    for square in board.pieces(chess.PAWN, chess.WHITE):
        white_files[chess.square_file(square)] = True
    for square in board.pieces(chess.PAWN, chess.BLACK):
        black_files[chess.square_file(square)] = True
    return white_files, black_files


def king_safety_score(board: chess.Board, colour: chess.Color, own_pawn_files: list[bool]) -> int:
    """Midgame-only bonus for pawns sheltering the king and penalty for open files beside it.
    Same values and logic as V15/V18; the open-file check now reads the shared `own_pawn_files`
    instead of rebuilding its own, but the shield-rank check still needs each pawn's actual
    square, so it keeps its own (at most 8-pawn) loop.
    """
    ...
    for square in board.pieces(chess.PAWN, colour):
        ...
        if (pawn_rank - king_rank) * forward > 0:
            shield_pawns += 1

    score = shield_pawns * KING_SHIELD_BONUS_MG
    for file in (king_file - 1, king_file, king_file + 1):
        if 0 <= file <= 7 and not own_pawn_files[file]:
            score -= KING_OPEN_FILE_PENALTY_MG
    return score


def rook_open_file_score(
    board: chess.Board,
    colour: chess.Color,
    white_pawn_files: list[bool],
    black_pawn_files: list[bool],
) -> int:
    """Flat bonus for each of this side's rooks with no pawn (semi-open) or no pawn of
    either colour (open) in front of it on its own file. Same values and logic as V16."""
    own_files = white_pawn_files if colour == chess.WHITE else black_pawn_files
    enemy_files = black_pawn_files if colour == chess.WHITE else white_pawn_files
    score = 0
    for square in board.pieces(chess.ROOK, colour):
        file = chess.square_file(square)
        if not own_files[file]:
            score += ROOK_OPEN_FILE_BONUS if not enemy_files[file] else ROOK_SEMI_OPEN_FILE_BONUS
    return score
...
def evaluate(board: chess.Board) -> int:
    ...
    Numerically V11/V17's tapered midgame/endgame evaluation plus V15's king-safety term
    (midgame only) plus V16's rook-on-open-file bonus (flat, both phases). Both added terms
    share one pass over each side's pawn files (`pawn_files()`) instead of scanning twice.
    """
    pieces = encode_board(board)
    mg_score, eg_score, phase = _eval_core(pieces, SIGNED_MG, SIGNED_EG, PHASE_WEIGHT_BY_CODE)

    white_files, black_files = pawn_files(board)
    mg_score += king_safety_score(board, chess.WHITE, white_files) - king_safety_score(
        board, chess.BLACK, black_files
    )
    rook_white = rook_open_file_score(board, chess.WHITE, white_files, black_files)
    rook_black = rook_open_file_score(board, chess.BLACK, white_files, black_files)
    mg_score += rook_white - rook_black
    eg_score += rook_white - rook_black
