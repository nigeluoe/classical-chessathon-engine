...
This is V17 (V11's tapered evaluator with the hot per-square reduction jitted by numba,
which was a real, measured 70.0% win over V11 -- see training/data/v17_vs_v11_20games.log)
with one addition: the king-safety term from V15 (pawn shield, open files near the king,
midgame only). V15 tried this same feature straight on V11 and it was only a 45.0% wash,
right at the edge of the team's own wash band -- not a clear win, but also not a clear
regression, and the cheapest of the four features tried that round (a 1.24x evaluate()
slowdown). The hypothesis this candidate tests: does that same feature clear the bar once it
is riding on the faster, jitted evaluator instead of V11's plain-Python one?

King safety itself is not jitted here. It stays a small, plain-Python loop over the king's own
nearby pawns (at most 8), added to the jitted core's signed midgame score before tapering, the
same way V17 leaves the bishop-pair bonus in plain Python after the jitted sum. Jitting a loop
this small and irregular (variable pawn count, an early-exit-free scan) is unlikely to pay for
its own call overhead -- exactly the lesson from V17's modest, mostly call-overhead-limited
1.13x microbenchmark speedup on the *much larger* 64-square core loop.
"""
...
# King-safety weights: midgame only, same values and reasoning as V15.
...
def king_safety_score(board: chess.Board, colour: chess.Color) -> int:
    """Midgame-only bonus for pawns sheltering the king and penalty for open files beside it.

    One pass over this side's own pawns (at most 8), no move generation, same as V15. Kept in
    plain Python rather than jitted: a loop this small and irregular is unlikely to earn back
    numba's per-call overhead, which is exactly why V17's much larger 64-square core loop only
    bought a modest 1.13x in isolation.
    """
...
def evaluate(board: chess.Board) -> int:
    ...
    Numerically V11/V17's tapered midgame/endgame evaluation (material plus placement,
    blended by a material-based phase estimate, plus a bishop-pair bonus) plus V15's
    king-safety term, added into the midgame side only so it fades out on its own as material
    is traded off. See `training/test_numba_eval.py` for the check that the jitted core here
    agrees with V11's pure-Python one before this addition.
    """
    pieces = encode_board(board)
    mg_score, eg_score, phase = _eval_core(pieces, SIGNED_MG, SIGNED_EG, PHASE_WEIGHT_BY_CODE)

    mg_score += king_safety_score(board, chess.WHITE) - king_safety_score(board, chess.BLACK)
