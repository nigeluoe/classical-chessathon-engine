"""V41: V35 search with referee-aligned repetition and conversion time.
...
def conversion_budgets(
    ...
    """Spend more of a safe reserve when a low-material advantage needs conversion."""
    if static < CONVERSION_SCORE or pieces > CONVERSION_PIECES:
        return soft, hard
    remaining = max(0.0, time_left_ms / 1000.0)
    usable = max(0.0, remaining - max(0.025, min(0.25, remaining * 0.05)))
...
def observe(board: chess.Board) -> None:
    global _AFTER_OUR_MOVE, _CHECK_STREAK
    key = board_key(board)
...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    soft, hard = conversion_budgets(
        soft, hard, time_left_ms, static, pieces, _CHECK_STREAK
    )
    chosen = legal[0]
