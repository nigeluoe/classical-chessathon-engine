# V41: Repetition-Aligned Conversion Candidate

V41 starts from V35's verified search-optimized engine and targets the conversion failures in
the saved 15-game V35-v40 match. It aligns search with the referee's claimable-threefold rule
and spends more clock when a low-material advantage needs deeper calculation. Evaluation and
the board/search substrate are otherwise unchanged. V41 is a candidate, not a claim of perfect
play or a guaranteed competition winner.

## V41 changes

- Search recognizes a draw when a legal next move would create the third occurrence, matching
  `python-chess` referee outcomes with `claim_draw=True`; TT cutoffs cannot bypass that check.
- When the side to move is at least 120 centipawns ahead with at most 16 pieces remaining, the
  soft and hard search windows can grow while retaining at least 15% of the usable clock.
- Repeated checks permit a slightly larger soft-window multiplier. Ordinary, equal, and losing
  positions retain V35's time allocation.
- The original drawn positions now choose `32.Qb5` at the 3.515-second clock and avoid
  `52...a4` at the 1.582-second clock when replayed with their exact histories.
...
```powershell
& .venv/Scripts/python.exe -u submissions/v41-conversion-candidate/dev/verify.py
& .venv/Scripts/python.exe -u submissions/v41-conversion-candidate/dev/external_arena.py `
  --opponent stockfish --games 2 --opening-offset 2 `
  --output training/data/v41/stockfish-fullclock-new
```
