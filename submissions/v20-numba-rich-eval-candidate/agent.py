...
This is V17 (V11's tapered evaluator with the hot per-square reduction jitted by numba) with
pawn structure (V14: doubled/isolated/passed pawns) and king safety (V15: pawn shield,
open files near the king) added -- but this time computed *inside* the jitted core instead of
in plain Python. V18 and V19 tried these same features riding on top of V17's faster material
core and both came back a wash or a regression, because the feature computation itself stayed
in interpreted Python: V17 only sped up the material-plus-piece-square sum, not the extra
chess-knowledge terms bolted on afterwards. This candidate tests whether jitting the whole
thing removes that bottleneck.
