...
computed inside the same jitted pass) with one robustness fix: a safer emergency fallback
move. Every version back through V11/V6 initialised `get_move()`'s `preferred` move to
`legal_moves[0]` -- an arbitrary move in whatever order python-chess's generator happens to
produce it -- and only replaced it once a full depth-1 `search_root()` call returned. If the
move budget is so tight that even depth 1 cannot finish (found while investigating a real,
reproducible blunder in a jitted-mobility experiment built on V20: in one messy middlegame
position, at a 3 s + 0.1 s local time control, V20 itself picked `legal_moves[0]` and hung a
bishop for nothing, confirmed at the interpreter with `get_move()` reporting `depth=0`),
`SearchTimeout` unwinds past the assignment and `get_move()` returns that untouched arbitrary
move -- which can be a piece left hanging, not merely a suboptimal one. Every depth *after*
the first is already safe: if depth N times out partway through, `preferred` simply keeps the
last depth that finished cleanly, a real (if shallow) search result. Only the very first
iteration was unguarded.

This candidate only recomputes `preferred` when `completed_depth == 0` after the search loop
exits, i.e. exactly the rare emergency case, not on every move: an earlier version of this
fix called the safer guess unconditionally up front and was a real, measured regression
(35.0% versus V20 over 20 games at a fast local control) purely from spending a little extra
time on every move for a safety net almost every move never needs. Computed lazily, this
candidate reruns `safe_first_guess()`: the same move-ordering heuristic `search_root()` uses
to choose its first candidate (a cached transposition-table move for this position if one
exists, else MVV-LVA/killer/history ranking), plus a one-shot `hangs_a_piece()` check --
simulate the move, then ask whether the opponent can capture on the destination square and
nothing of ours recaptures -- that plain move-ordering cannot break a tie on, since every
quiet move scores 0 against an empty history table. Since this only runs when the deadline is
already blown, its cost is irrelevant. No evaluation or search-tree logic changes; the
underlying evaluator is unchanged from V20 and is still covered by
`training/test_numba_rich_eval.py`.

How the rich core works
------------------------
Everything V17's `_eval_core` already did (material, piece-square placement, phase), plus, in
the same 64-square scan, per-file pawn counts for both colours and the rank extremes needed
for a passed-pawn check (the lowest white pawn rank and highest black pawn rank on each file),
each side's king square, and each side's bishop count. A second 64-square pass then scores
each pawn (doubled, isolated, passed) now that those file-level tallies are complete, and each
king's shelter (pawns on its own and adjacent files, ahead of it) and open-file exposure. All
of this happens inside one `@njit` function; the plain-Python wrapper only encodes the board
and applies the final phase blend, same as V17.

Correctness is checked before any game is played by `training/test_numba_rich_eval.py`,
against a plain-Python reference that combines V11's material/placement, V14's pawn-structure
function, and V15's king-safety function exactly as this candidate is supposed to.
"""
...
def _eval_core_rich(
    ...
    """Material, placement, pawn structure, king safety, and bishop pair, all in one jitted
    pass plus a second one over the (already-known-few) pawns. See the module docstring for
    what each per-file tally is for.
    """
    mg_score = 0
    eg_score = 0
    phase = 0
...
def evaluate(board: chess.Board) -> int:
    ...
    plus the same bishop-pair bonus, all computed inside `_eval_core_rich`. See
    `training/test_numba_rich_eval.py` for the check that this matches a plain-Python
    reference built from V11/V14/V15's own functions before any game was played.
    """
...
def hangs_a_piece(board: chess.Board, move: chess.Move) -> bool:
    """A cheap, one-shot 'does this walk a piece onto a square nothing of ours defends, that
    the opponent can capture on their next move' check. Deliberately not part of
    move_order_score/ordered_moves (which run on the search's hot path, at every node): this
    is only ever called from get_move() itself, once per call, to pick a safer initial/
    emergency-fallback guess than move-ordering scores alone can give when no capture is
    available to rank above it (a quiet move with an empty history table ties every other
    quiet move at score 0). Checking attackers/defenders on the board *before* the move would
    be wrong: the moving piece's own current attack on its destination is exactly what makes
    a non-capture slide legal in the first place, so it would always look "defended" by
    itself. Push the move first so is_attacked_by only ever sees other pieces.
    """
    ...
    try:
        ...
        if not board.is_attacked_by(opponent, move.to_square):
            return False
        return not board.is_attacked_by(mover, move.to_square)
    finally:
        board.pop()


def safe_first_guess(board: chess.Board, tt_move: chess.Move | None) -> chess.Move:
    """The same move-ordering heuristic search_root() uses to pick its first candidate, but
    with quiet moves that hang a piece for nothing moved to the back first. Used to seed
    get_move()'s initial `preferred` value, which is what an emergency return uses if even
    depth 1 cannot finish before the deadline (see get_move()'s docstring note)."""
    moves = ordered_moves(board, 0, tt_move)
    safe_moves = [move for move in moves if not hangs_a_piece(board, move)]
...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    if completed_depth == 0:
        # Even depth 1 could not finish before the deadline, so `preferred` is still the
        # arbitrary `legal_moves[0]` set above -- whatever order python-chess's move generator
        # happens to produce, not a considered guess (every depth after the first is already
        # safe: if depth N times out partway, `preferred` simply keeps the last depth that
        # finished cleanly, a real search result). This branch is only reached in genuine time
        # trouble, so the deadline is already blown either way; spending a little more time
        # here to avoid returning a move that hangs a piece for nothing is worth it, and this
        # is the only place that cost is ever paid -- every normal move that completes at
        # least depth 1 never reaches here.
        fallback_entry = TRANSPOSITION_TABLE.get(position_key(board))
        preferred = safe_first_guess(board, fallback_entry.best_move if fallback_entry else None)
