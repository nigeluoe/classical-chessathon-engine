...
V33 is a deliberately conservative strength experiment built from the validated V29 core.
It adds selective pruning only in non-principal-variation nodes, where the engine is proving a
move cannot beat an already-established score, and spends a little more of the competition
clock while retaining a fixed reserve. The principal variation, checks, captures, and evasions
keep V29's full search treatment.
...
# Selective pruning is restricted to zero-window (non-PV) nodes. This is materially safer than
# applying it to the principal variation: the first/best candidate line is still searched with
# V29's full logic, while poorly ordered quiet alternatives consume fewer nodes.
RFP_MAX_DEPTH = 6
RFP_MARGIN_PER_DEPTH = 115
LMP_MAX_DEPTH = 4
LMP_THRESHOLD = np.array([0, 3, 6, 10, 16], dtype=np.int64)
...
def negamax(
    ...
    # Reverse futility pruning: at a non-PV node with a narrow beta bound, a static score that
    # already clears beta by a depth-scaled safety margin is enough to cut off. Never use it in
    # check or on a principal-variation node, where tactical precision matters more than speed.
    ...
    for i in range(count):
        ...
        # Late-move pruning only discards quiet, non-checking moves after several such moves
        # have already failed at a zero-window node. It never applies to checks, captures,
        # evasions, or the principal variation.
        if (
            not is_pv
            and not in_check
            and is_quiet
...
def move_budget_seconds(fullmove_number: int, time_left_ms: int) -> float:
    """Convert the remaining competition clock into a safe per-move search allowance.

    V29 caps every move at three seconds. The platform gives a 120-second base clock plus a
    half-second increment, so V33 may use modestly more in critical early/middlegame positions
    without borrowing the final reserve needed for forced endgames or slow hardware.
    """
    seconds_left = max(0.0, time_left_ms / 1000)
    reserve_seconds = 10.0
    expected_moves_left = max(14, 42 - fullmove_number)
    if seconds_left <= reserve_seconds:
        # Local arena tests commonly use a 3-10 second base clock. A competition-sized reserve
        # would consume that entire clock, so retain V29's tested allocation under short clocks.
        return max(0.02, min(3.0, seconds_left / expected_moves_left + 0.05))
    spendable = max(0.0, seconds_left - reserve_seconds)
    budget = spendable / expected_moves_left * 1.15 + 0.05
    return max(0.02, min(4.25, budget))
