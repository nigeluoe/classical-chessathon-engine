"""V53: depth-consistent classical PVS chess engine.
...
def search_node(
    ...
    replace = tt_keys[slot] != key or requested_depth >= tt_depths[slot] or best_score >= beta
    if replace:
        tt_keys[slot] = key
        # Probe and storage both use the caller's requested depth. The check
        # extension is inherent to this position and must not count twice
        # when a later, deeper request probes the same entry.
        tt_depths[slot] = requested_depth
    ...
    if len(legal) > 1 and hard >= 0.004:
        ...
        if move is not None:
            ...
            if candidate in legal:
                chosen = candidate
    print(f"v53 depth={depth} nodes={nodes} score={score} move={chosen.uci()}", flush=True)
    board.push(chosen)
