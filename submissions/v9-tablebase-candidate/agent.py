...
# Syzygy WDL files are permitted shipped data and load before the game clock starts.
TABLEBASE = chess.syzygy.open_tablebase(
    str(Path(__file__).with_name("tablebases")), load_wdl=True, load_dtz=False
)
...
def tablebase_move(board: chess.Board) -> chess.Move | None:
    """Choose an exact WDL move when our compact Syzygy set covers the position."""
    ...
    best_wdl = -3
    for move in ordered_moves(board, 0, None):
        board.push(move)
        try:
...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    exact_move = tablebase_move(board)
    if exact_move is not None:
        print(f"tablebase move={exact_move.uci()}")
        return exact_move.uci()
