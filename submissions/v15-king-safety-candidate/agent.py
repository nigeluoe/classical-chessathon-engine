...
addition: a king-safety term. It rewards pawns sheltering the king and penalises open files
next to it, added only into the midgame total (not the endgame one), so the term naturally
fades out as material is traded off without any extra tapering logic -- exactly when king
safety stops mattering and king activity (already captured by the endgame king piece-square
table) takes over.

Built fresh from V11, not stacked on v14 (pawn structure): v14 was a clear, real regression
(22.5% versus V11, training/data/v14_vs_v11_20games.log) because its ~2.2x evaluate()
slowdown cost more search depth than the extra accuracy bought back. The same risk applies
here, so this candidate is deliberately as cheap as possible: one pass over the king's own
pawns (at most 8) per side, no move generation, gated on a real speed measurement before any
game was played.
"""
...
# King-safety weights: midgame only (see module docstring for why no endgame counterpart).
...
def king_safety_score(board: chess.Board, colour: chess.Color) -> int:
    ...
    One pass over this side's own pawns (at most 8), no move generation: cheap by
    construction, following the lesson from v13 (mobility) and v14 (pawn structure), both
    real regressions caused by evaluate() becoming too slow to afford the search depth lost.
    """
...
def evaluate(board: chess.Board) -> int:
    ...
    A king-safety term (pawn shelter, open files) is added only into the midgame side, so it
    fades out on its own as material is traded off, without extra tapering logic. The blended
    result is then interpolated by a material-based phase estimate.
    """
    mg_white = mg_black = 0
    eg_white = eg_black = 0
    phase = 0
    ...
    mg_white += king_safety_score(board, chess.WHITE)
    mg_black += king_safety_score(board, chess.BLACK)
