...
Status: locally verified with a positive but modest internal result against V41. The 15-game
sample is not sufficient to establish a reliable Elo gain, and external-engine validation is
still pending.

## V41 self-play diagnosis

Frozen V41 played itself for 15 games at 10s + 0.1s on the same five paired opening families.
The candidate seat scored **+6 =4 -5 (53.3%)**, appropriately close to even for identical source.
There were 11 checkmates, two threefold repetitions, one insufficient-material draw, and one
fifty-move draw, with no flags, crashes, illegal moves, or void games. Evidence is in
`training/data/v41/v41_selfplay_15games/`.

The drawn endings were not established missed wins: one was rook versus bishop, one simplified
to bare kings, and two repeated in roughly balanced positions. The decisive logs exposed a more
general asymmetry. Once a low-material score reached +120, V41 gave the converting side extra
depth, but the defending side at -120 retained the shallow base budget. Tactical search scores
could also show danger that the static score missed.

## V42 change and measured probes

V42 makes V41's bounded critical-position budget symmetric and retains the previous completed
search score for one turn when it has greater magnitude than the current static score. Ten saved
defensive positions were replayed with exact histories and clocks. Triggered probes gained 2-6
completed depths; sampled choices remained stable, providing deeper confirmation rather than a
cherry-picked move change.

## V42 versus frozen V41

On the same 15-game, 10s + 0.1s opening set, V42 scored **+7 =3 -5 (56.7%)**. All 12 decisive
games ended by checkmate and all three draws by repetition. There were no flags, crashes, illegal
moves, or void games. This is a +2 net-game result and therefore encouraging but inconclusive.
Complete evidence is in `training/data/v42/v42_vs_v41_15games/summary.json`.

## Verification and package

Ruff and strict mypy pass. Five exact perft positions, 1,494 randomized legal/make-unmake/hash/
evaluation checks, TT guards, repetition and clock regressions, and 11,376 independent legal-SEE
captures pass. Verification import took 35.03s. Arena-tested runtime hashes are
`agent.py=7db5cd4ff7e43c590827fb20d2bc713e874f5023d3439f2c74202d270129b8ce` and
`core.py=64d4b518d7dba9581183d913d46469be32c9f377ce257205d1a4c73c1c1bc2f1`.

`submissions/agent-v42-balanced-critical.zip` contains exactly `agent.py` and `core.py` at the
archive root (77,778 bytes uncompressed, 22,249 bytes compressed). Archive SHA-256:
`06e721eddbeb04324298a7292ab145b963bfd6ff6b7175bb60dec9b943d84f4e`.

## Historical V41 results
