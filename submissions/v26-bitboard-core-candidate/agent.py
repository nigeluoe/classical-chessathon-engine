...
Every previous version in this repo (V6 through V25) still generates moves through
python-chess's object-oriented `chess.Board`; only the evaluation function's hot loop was ever
jitted (V17, V20). `docs/IDEAS.md` says this gets "thousands, not millions" of nodes per
second, and that is the real ceiling on every eval tweak this repo has tried. This candidate
replaces the board representation itself: 12 piece bitboards plus derived occupancy, side to
move, castling rights, en-passant square, and halfmove clock, with move generation, make/unmake,
search, and evaluation all running under `@njit` -- no `python-chess` object anywhere in the hot
path. `python-chess` is used only at the edges: to parse the input FEN once per `get_move()`
call and to format the chosen move back to a UCI string.
...
as a fixed array (no dict, no Python objects), MVV-LVA + TT-move ordering, and a quiescence
search on captures and promotions -- all `@njit`. This is a deliberately smaller slice of
V20's search than the full plan asks for eventually: no null-move pruning, LMR, futility
pruning, aspiration windows, or killer/history tables yet. Zobrist hashing is incremental,
updated inside `apply_move_hashed`/`unmake_move_hashed` (verified against a from-scratch
recompute in `training/test_bitboard_makeunmake.py`).
...
def sliding_attacks(s: int, occ: np.uint64, dir_df: np.ndarray, dir_dr: np.ndarray) -> np.uint64:
    ...
    board edge or a blocker (inclusive of the blocker itself). Simple and obviously correct,
    not a magic-bitboard lookup -- see the module docstring for why that trade was made."""
    f0 = s % 8
    r0 = s // 8
    bb = np.uint64(0)
    for i in range(4):
...
# MVV-LVA + TT-move ordering, and quiescence search on captures/promotions. A smaller slice
# of V20's search than the full plan (no null-move, LMR, futility, aspiration, killers, or
# history yet) -- see the module docstring for why.
# --------------------------------------------------------------------------------------------
...
def negamax(
    ...
    count = gen_legal_moves(boards, meta, buf)
    if count == 0:
        if is_in_check(boards, meta):
            return -MATE_SCORE + ply
        return 0

    order_moves(buf, count, tt_best)
    undo = np.empty(6, dtype=np.int64)
...
def search_root(
    ...
    order_moves(root_moves, root_count, tt_best)
    undo = np.empty(6, dtype=np.int64)
    best_score = -MATE_SCORE
...
class Engine:
    """Owns the transposition table so it survives between moves in the same game (the
    platform keeps the process alive for a whole game)."""
    ...
    def search(
        ...
        for depth in range(1, max_depth + 1):
            ...
            move, score = search_root(
                boards, meta, hash_arr, depth, -MATE_SCORE, MATE_SCORE,
                node_count, deadline, timed_out,
                self.tt_key, self.tt_depth, self.tt_score, self.tt_flag, self.tt_move,
                root_copy, root_count, tt_best,
            )
