...
# Reverse futility pruning (a.k.a. static null-move pruning) and late-move pruning: standard,
# widely-documented techniques not present in V20, so ported from the community's usual
# description (chessprogramming.org/Reverse_Futility_Pruning) rather than from this repo's own
# prior code, with values chosen from the typical published range rather than tuned via a
# parameter sweep this round -- said plainly rather than implied as measured.
#
# Reverse futility pruning: at shallow remaining depth, if the static evaluation already beats
# beta by more than a depth-scaled margin, assume a real search would not find anything to
# change that and cut off immediately without searching any moves at all.
RFP_MAX_DEPTH = 8
RFP_MARGIN_PER_DEPTH = 120

# Late-move pruning: at shallow remaining depth, once enough quiet moves have already been
# tried without raising alpha, skip the rest of the quiet moves outright (not just reduce them
# via LMR) -- move ordering means the moves left at that point are the ones least likely to
# matter. LMP_THRESHOLD[depth] (depth 0 unused) is how many quiet moves may be tried before the
# rest are skipped at that depth; a common shape is roughly cubic in depth so deeper searches
# tolerate more quiet moves before pruning.
LMP_MAX_DEPTH = 8
LMP_THRESHOLD = np.array([0, 4, 6, 10, 15, 21, 28, 36, 45], dtype=np.int64)
...
def negamax(
    ...
    # Reverse futility pruning (static null-move pruning): computed once here and reused below
    # for the existing depth==1 futility check, so it is never evaluated twice at the same
    # node. If the position already beats beta by more than a depth-scaled margin, even
    # allowing for real search error, assume that holds and cut off without searching any
    # moves -- standard technique, not present in V20 (see the constants above).
    static_eval = -MATE_SCORE
    if not in_check and depth <= RFP_MAX_DEPTH:
        static_eval = eval_from_arrays(boards, meta)
        if beta < MATE_SCORE - 100 and static_eval - RFP_MARGIN_PER_DEPTH * depth >= beta:
            return beta
    ...
    lmp_threshold = LMP_THRESHOLD[depth] if depth <= LMP_MAX_DEPTH else MAX_MOVES
    ...
    for i in range(count):
        ...
        # Late-move pruning: once enough quiet moves have already been tried at this node
        # without raising alpha, skip the rest outright rather than reduce them via LMR --
        # move ordering means what is left is the least likely to matter. Never applied when
        # the current position is in check (every evasion needs considering) or to a move that
        # itself gives check (that is exactly the kind of quiet move worth keeping).
        if (
            not in_check
            and is_quiet
            and not gives_check
