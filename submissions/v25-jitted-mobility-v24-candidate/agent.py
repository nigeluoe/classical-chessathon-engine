...
This is V24 (V20's jitted material/placement/pawn-structure/king-safety core, plus a safer
emergency-fallback move -- see V24's own module docstring for that fix) with a jitted
mobility term added on top. V23 tried the same idea directly on V20 and the result was
confusing: a 55.0%/50.0% (wash) head-to-head against V20 but a striking, reproducible
weakness playing White specifically against a weak baseline (57.5% versus the baseline's
usual ~90-100%). Investigating that anomaly is what turned up V24's bug in the first place
(V20 itself, not just V23, could return `legal_moves[0]` -- an arbitrary move, once
confirmed hanging a whole bishop for nothing -- whenever even depth 1 timed out): V23's
mobility term made `evaluate()` measurably (if only slightly, ~3%) slower, which plausibly
pushed already-tight positions over that timeout cliff somewhat more often than V20 hit it,
confounding the mobility comparison itself. This candidate re-tests the same mobility term on
top of V24 instead, so the comparison against the current best is no longer entangled with
that shared bug.
