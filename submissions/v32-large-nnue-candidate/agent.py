...
NNUE_SQUARES = 64
NNUE_FEATURE_COUNT = KING_BUCKETS * PIECE_CATEGORIES * NNUE_SQUARES  # 20,480
...
def king_bucket_and_mirror(king_sq: int, persp_is_black: bool) -> tuple[int, bool]:
    """32-way mirrored king bucket matching V32 training features.

    Files a-d mirror onto e-h, while all eight ranks remain distinct. This is
    deliberately the same transform as `training/v10_nnue.py`, from which V32
    imports its sparse feature encoder.
...
def compute_accumulators(boards: np.ndarray) -> np.ndarray:
    ...
    test."""
    acc = np.empty((2, NNUE_W1.shape[1]), dtype=np.int32)
...
def nnue_eval(acc: np.ndarray, side_to_move: int) -> int:
    ...
    for i in range(h):
        u = us[i] * NNUE_W1_SCALE
        x[i] = 0.0 if u < 0.0 else (1.0 if u > 1.0 else u)
        t = them[i] * NNUE_W1_SCALE
...
@njit(cache=False)
def static_eval(boards: np.ndarray, meta: np.ndarray, acc: np.ndarray) -> int:
    """Use trained V32 when present; otherwise retain V29's proven evaluator."""
    if NNUE_ENABLED:
...
def load_nnue_weights() -> NnueWeights:
    """Load V32's team-trained, quantised artifact or enable the V29 fallback.

    The feature-transformer table remains int16 in memory. Its accumulator is
    updated as int32 during search; only the tiny dense layers are dequantised
    once at import. A missing or malformed artifact never produces random
    play: `NNUE_ENABLED` remains false and search uses V29's evaluator.
    """
    weights_path = _Path(__file__).resolve().parent / "weights" / "nnue_v32_int.npz"
    if weights_path.is_file():
        with np.load(weights_path) as data:
            required = {
                "embedding", "embedding_scale", "w2", "w2_scale", "b2", "w3", "w3_scale",
                "b3", "w4", "w4_scale", "b4", "centipawn_scale",
            }
            if required.issubset(data.files):
                w1 = np.ascontiguousarray(data["embedding"], dtype=np.int16)
                w2 = np.ascontiguousarray(
                    data["w2"].astype(np.float32) * float(data["w2_scale"][0])
                )
                w3 = np.ascontiguousarray(
                    data["w3"].astype(np.float32) * float(data["w3_scale"][0])
                )
                w4 = np.ascontiguousarray(
                    data["w4"].astype(np.float32) * float(data["w4_scale"][0])
                )
                if (
                    w1.shape == (NNUE_FEATURE_COUNT, ACCUMULATOR_SIZE)
                    and w2.shape == (NNUE_HIDDEN1, ACCUMULATOR_SIZE * 2)
                    and w3.shape == (NNUE_HIDDEN2, NNUE_HIDDEN1)
                    and w4.shape == (1, NNUE_HIDDEN2)
                ):
                    return (
                        True,
                        w1,
                        float(data["embedding_scale"][0]),
                        w2,
...
(
    NNUE_ENABLED, NNUE_W1, NNUE_W1_SCALE, NNUE_W2, NNUE_B2, NNUE_W3, NNUE_B3, NNUE_W4,
    NNUE_B4, NNUE_CENTIPAWN_SCALE,
) = load_nnue_weights()
...
def quiescence(
    ...
    if qdepth >= MAX_QUIESCENCE_PLY or ply >= MAX_PLY:
        return static_eval(boards, meta, acc)

    in_check = is_in_check(boards, meta)
    stand_pat = static_eval(boards, meta, acc)
    if not in_check:
        if stand_pat >= beta:
            return beta
        if stand_pat > alpha:
    ...
    undo = np.empty(6, dtype=np.int64)
    undo_acc = np.empty((2, ACCUMULATOR_SIZE), dtype=np.int32)
    for i in range(count):
...
def negamax(
    ...
    static_score = static_eval(boards, meta, acc) if depth == 1 else -MATE_SCORE
    ...
    for i in range(count):
        ...
        if (
            ...
            and is_quiet
            and not gives_check
            and static_score + FUTILITY_MARGIN <= alpha
        ):
            unmake_move_nnue(boards, meta, undo, hash_arr, acc, undo_acc)
            continue
...
class Engine:
    ...
    def search(
        ...
        if NNUE_ENABLED:
            ...
            # Search signatures stay uniform while the model artifact is absent.
            acc = np.zeros((2, ACCUMULATOR_SIZE), dtype=np.int32)
