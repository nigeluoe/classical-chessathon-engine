"""V49: V48 plus a longer-game clock reserve schedule (experimental).

Only the perft-tested board representation and legal move substrate live in
``core.py``.  Move selection here is V48's implementation (itself V44/V43's
search, unchanged) plus one additional, real-behaviour-changing tweak to
``time_budget``: past move 60, ``moves_left`` grows instead of staying
floored at 18, so per-move spending gradually tapers in a long, unresolved
game instead of continuing to spend at the same rate all the way to move
190+. This directly targets a real rated-game loss (round 97, 198 plies,
both sides down to 3-8 real seconds by move 184) rather than the ceiling-only
change V48 made, but unlike V48's change it is not safe purely "by
construction" -- it changes how much a normal, non-desperate long game
spends per move, and this round's remaining time did not allow validating it
at the scale (many long real games) that would be needed to be confident it
is a net improvement rather than a new, different risk. See this directory's
RESULTS.md for exactly what was and was not verified before treating this as
anything more than a documented, reasoned hypothesis.
"""
...
def time_budget(time_left_ms: int, fullmove: int) -> tuple[float, float]:
    ...
    # V43/V44/V48 hold moves_left at a flat floor of 18 forever past move 20.
    # A real rated-game loss this round (round 97, 198 plies) shows both
    # sides down to 3-8 real seconds by move 184 -- a still-unresolved long
    # game had drained its own clock by the time a critical decision needed
    # real depth. Dividing by only 18 "moves left" no matter how long the
    # game has already run keeps spending at the same rate deep into a long
    # game instead of husbanding more reserve for a later critical moment.
    # Past move 60, grow moves_left further (capped at 60) so per-move
    # spending tapers down the longer an unresolved game runs, leaving more
    # of the bank available exactly when a long shuffling phase finally
    # resolves. This changes real behaviour (unlike V48's ceiling-raise) and
    # was not validated at scale with real long games given this round's
    # time budget -- see this directory's RESULTS.md for the honest caveat.
    if fullmove > 60:
        moves_left = min(60, moves_left + (fullmove - 60) // 3)
    # V43/V44 capped these at a flat 4.0s/7.0s regardless of banked clock.
    # That only ever binds once real usable time is large relative to
    # moves_left -- unreachable at this project's usual 10s+0.1s local
    # benchmark, but routinely reachable in a long game at the real
    # 120s+0.5s platform control once increments have banked extra time.
    # The proportional fractions below (usable/moves_left, usable*0.35,
    # soft*2.25) are exactly V43/V44's own safety margins, unchanged; only
    # the fixed-second ceiling that used to override them is raised, so
    # this can only match or increase the time spent versus V43/V44, never
    # reduce it.
    soft = min(20.0, usable / moves_left + 0.025)
    hard = min(35.0, usable * 0.35, soft * 2.25)
    ...
    if len(legal) > 1 and hard >= 0.004:
        ...
        if move is not None:
            ...
            if candidate in legal:
                chosen = candidate
    print(f"v49 depth={depth} nodes={nodes} score={score} move={chosen.uci()}", flush=True)
    board.push(chosen)
