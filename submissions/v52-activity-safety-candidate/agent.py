"""V52: activity-aware classical PVS chess engine.
...
MOBILITY_MG = np.array([0, 4, 5, 2, 1, 0], dtype=np.int64)
MOBILITY_EG = np.array([0, 3, 4, 4, 2, 0], dtype=np.int64)
MOBILITY_BASE = np.array([0, 4, 6, 7, 12, 0], dtype=np.int64)
ATTACK_WEIGHT = np.array([0, 2, 2, 3, 5, 0], dtype=np.int64)


@njit(cache=False)
def activity_adjustment(boards: np.ndarray) -> int:
    white_pawns, black_pawns = boards[c.WP], boards[c.BP]
    white_attacks = (white_pawns & ~c.FILE_MASKS[0]) << np.uint64(7)
    white_attacks |= (white_pawns & ~c.FILE_MASKS[7]) << np.uint64(9)
    black_attacks = (black_pawns & ~c.FILE_MASKS[0]) >> np.uint64(9)
    black_attacks |= (black_pawns & ~c.FILE_MASKS[7]) >> np.uint64(7)
    white_occ, black_occ = c.occ_white(boards), c.occ_black(boards)
    occupied = white_occ | black_occ
    mg = eg = phase = 0
    for side in range(2):
        base = side * 6
        sign = 1 if side == c.WHITE else -1
        own = white_occ if side == c.WHITE else black_occ
        enemy_pawn_attacks = black_attacks if side == c.WHITE else white_attacks
        king = c.king_square(boards, side != c.WHITE)
        zone = c.KING_ATTACKS[king] | (np.uint64(1) << np.uint64(king))
        units = attackers = 0
        own_pawns = boards[base]
        enemy_pawns = boards[(1 - side) * 6]
        for kind in range(1, 5):
            pieces = boards[base + kind]
            phase += c.popcount64(pieces) * c.PHASE_WEIGHT[kind]
            while pieces:
                ...
                if kind == 1:
                    ...
                    if kind == 2 or kind == 4:
                        attacks |= c.bishop_attacks(square, occupied)
                    if kind == 3 or kind == 4:
                        attacks |= c.rook_attacks(square, occupied)
                safe = attacks & ~own & ~enemy_pawn_attacks
                mobility = c.popcount64(safe) - MOBILITY_BASE[kind]
                mg += sign * MOBILITY_MG[kind] * mobility
                eg += sign * MOBILITY_EG[kind] * mobility
                hits = c.popcount64(attacks & zone)
                if hits:
                    attackers += 1
                    units += ATTACK_WEIGHT[kind] * hits
                if kind == 3:
                    file = c.FILE_MASKS[square % 8]
                    rank = square // 8 if side == c.WHITE else 7 - square // 8
                    enemy_rank = king // 8 if side == c.WHITE else 7 - king // 8
                    seventh = np.uint64(0x00FF000000000000 if side == c.WHITE else 0xFF00)
                    if rank == 6 and (enemy_rank >= 6 or enemy_pawns & seventh):
                        mg += sign * 15
                        eg += sign * 25
                    if not (own_pawns & file):
                        mg += sign * (15 if not enemy_pawns & file else 8)
                        eg += sign * 6
        # Reward coordinated threats, rather than an unsupported queen sortie.
        if attackers >= 2:
            danger = min(300, units * units // 4)
            if not boards[base + c.WQ]:
                danger //= 4
            mg += sign * danger
    phase = min(24, phase)
...
def static_eval(boards: np.ndarray, meta: np.ndarray) -> int:
    ...
    white_adjustment = activity_adjustment(boards)
...
def cached_eval(
    ...
    # Static evaluation depends only on the board and side, not search history
    # or the halfmove clock. Draw and repetition checks remain outside this cache.
    slot = int(key & EVAL_MASK)
    if eval_keys[slot] == key and eval_values[slot] != INF:
        return int(eval_values[slot])
    value = static_eval(boards, meta)
    eval_keys[slot] = key
    eval_values[slot] = value
    return value


@njit(cache=False)
def gen_tactical_moves(boards: np.ndarray, meta: np.ndarray, buf: np.ndarray) -> int:
    white = meta[0] == c.WHITE
    own = c.occ_white(boards) if white else c.occ_black(boards)
    enemy = c.occ_black(boards) if white else c.occ_white(boards)
    occupied = own | enemy
    # Retain the established pawn path for all en-passant and underpromotion
    # cases. Only non-pawn quiet moves are avoided altogether.
    count = c.gen_pawn_moves(boards, meta, buf, 0, white)
    kept = 0
    for index in range(count):
        move = int(buf[index])
        _f, _t, _p, captured, promo, _ep, _d, _k, _q = c.unpack_move(move)
        if captured != c.NO_PIECE or promo:
            buf[kept] = move
            kept += 1
    ...
    for kind in range(1, 6):
        ...
        while pieces:
            ...
            attacks &= enemy
            while attacks:
                target, attacks = c.pop_lsb(attacks)
                captured = c.piece_at(boards, target)
                buf[kept] = c.make_move_int(
                    square,
                    target,
                    piece,
                    captured,
...
def attacks_enemy_major(boards: np.ndarray, square: int, moving_white: bool) -> bool:
    ...
    """
    enemy_major = (boards[c.BQ] | boards[c.BR]) if moving_white else (boards[c.WQ] | boards[c.WR])
    if not enemy_major:
        return False
...
def qsearch(
    ...
    stand = cached_eval(boards, meta, hash_value[0], eval_keys, eval_values)
    if ply >= MAX_PLY - 1:
        return stand
    if not in_check:
        if stand >= beta:
    ...
    undo = undo_stack[ply]
    count = (
        c.gen_pseudo_moves(boards, meta, moves)
        if in_check
        else gen_tactical_moves(boards, meta, moves)
    )
    kept = 0
    if in_check:
        for index in range(count):
            ...
            scores[index] = (
                32 * PIECE_VALUE[captured % 6] - PIECE_VALUE[piece % 6]
    ...
    for index in range(count):
        ...
        frm, to, _piece, captured, promo, ep, _dbl, _ck, _cq = c.unpack_move(move)
...
def search_node(
    ...
    path[path_index] = hash_value[0]
    if ply and (
        repeated(path, path_index, meta[3], root_index, barrier)
        or meta[3] >= 100
        or drawn_material(boards)
    ):
        return 0
    if ply >= MAX_PLY - 1:
        return cached_eval(boards, meta, hash_value[0], eval_keys, eval_values)
    ...
    if in_check:
        depth += 1
    static = -INF if in_check else cached_eval(boards, meta, hash_value[0], eval_keys, eval_values)
    ...
    non_pawns = (
        boards[c.WN] | boards[c.WB] | boards[c.WR] | boards[c.WQ]
        if white
        else boards[c.BN] | boards[c.BB] | boards[c.BR] | boards[c.BQ]
    )
    if (
        allow_null
        and not pv_node
    ...
    for index in range(count):
        ...
        major_threat = (
            quiet
            and not gives_check
            and (depth <= 3 or ply == 0)
            and attacks_enemy_major(boards, to, white)
        )
        legal_count += 1
        if quiet:
            quiet_count += 1
        ...
        # At most two quiet threat extensions along any search path. This
        # exposes the forced reply without allowing endless queen chasing.
        extend = int(major_threat and depth <= 2 and extensions_left > 0)
        child_extensions = extensions_left - extend
        child_depth = depth - 1 + extend
        reduction = 0
        if (
            depth >= 3
            and legal_count >= 4
...
class Engine:
    def __init__(self) -> None:
        self.eval_keys = np.zeros(1 << 18, dtype=np.uint64)
        self.eval_values = np.full(1 << 18, INF, dtype=np.int32)
    ...
    if len(legal) > 1 and hard >= 0.004:
        ...
        if move is not None:
            ...
            if candidate in legal:
                chosen = candidate
    print(f"v52 depth={depth} nodes={nodes} score={score} move={chosen.uci()}", flush=True)
    board.push(chosen)
