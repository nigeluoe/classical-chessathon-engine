"""V48: V44's warm-up-safe classical PVS engine, plus real-clock utilization.

Only the perft-tested board representation and legal move substrate live in
``core.py``.  Move selection here is V44/V43's implementation unchanged apart
from two mechanical, non-behavioural-logic changes documented in this
directory's RESULTS.md: (1) a larger transposition table (more headroom for
the very long, repetitive games this project's real rated-game losses showed
are a real weak spot -- more TT slots means fewer collisions late in a long
game), and (2) removing an artificially low absolute ceiling on per-move
search time so that, at time controls far longer than this project's usual
10s+0.1s local benchmark (in particular the real 120s+0.5s platform control),
banked clock time can actually be spent instead of being capped at a few
seconds regardless of how much is available. Both changes are safe by
construction: the transposition table is sized, not restructured, and the
time formula's existing proportional safety fractions (a share of usable
clock, never all of it) are unchanged -- only the fixed-second ceilings that
sat below those fractions at long time controls were raised. Iterative
deepening, principal-variation search, and all pruning/reduction logic are
byte-for-byte V43/V44's own conservative selective pruning and explicit
clock management. The engine does not ponder; the competition process is
suspended while the opponent thinks.
"""
    ...
    if len(legal) > 1 and hard >= 0.004:
        ...
        if move is not None:
            ...
            if candidate in legal:
                chosen = candidate
    print(f"v48 depth={depth} nodes={nodes} score={score} move={chosen.uci()}", flush=True)
    board.push(chosen)
