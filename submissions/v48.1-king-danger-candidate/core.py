"""V48 board representation, move generation, hashing, and exchange evaluation.
...
# King-danger term (v48.1): a cheap, standard "attacker count near the king" safety index,
# midgame-only like the pre-existing shield/open-file term, added because that pre-existing
# term (pawn shield + open file only) does not react at all to enemy pieces massing near the
# king -- exactly the gap behind the two-knight sacrifice loss this candidate targets
# (submissions/README.md's V48 section, round-98 diagnosis: "a quiet piece buildup... the same
# class of blind spot... a trap that plays out as a slower positional squeeze rather than a
# capture sequence is still caught only by search depth, if at all"). For each side, every enemy
# knight/bishop/rook/queen that attacks a square in the king's own 3x3 zone (KING_ATTACKS[king]
# plus the king's own square) adds weighted "attack units"; the total (capped) indexes a
# nonlinear centipawn penalty, applied only to the midgame score so it fades out with material
# the same way the existing shield/open-file term already does. Reuses the same O(1) magic
# bitboard attack lookups SEE already relies on, so the added cost is proportional to the number
# of enemy minor/major pieces (typically <= 7 per side), not a new move generation or search.
KNIGHT_ATTACK_UNITS = 2
BISHOP_ATTACK_UNITS = 2
ROOK_ATTACK_UNITS = 3
QUEEN_ATTACK_UNITS = 5
KING_DANGER_MAX_UNITS = 8
KING_DANGER_MG = np.array([0, 0, 10, 25, 45, 70, 100, 135, 175], dtype=np.int32)
...
def eval_core(
    ...
    for side in range(2):
        ...
        if boards[offset + 5]:
            ...
            # King-danger: weighted count of enemy minor/major pieces attacking a square in
            # this king's own 3x3 zone, indexed into a nonlinear centipawn penalty. See the
            # KING_DANGER_MG constant's own comment above for why this was added.
            king_zone = KING_ATTACKS[king] | (np.uint64(1) << np.uint64(king))
            units = 0
            enemy_pieces = boards[enemy + 1]
            while enemy_pieces:
                sq, enemy_pieces = pop_lsb(enemy_pieces)
                if KNIGHT_ATTACKS[sq] & king_zone:
                    units += KNIGHT_ATTACK_UNITS
            enemy_pieces = boards[enemy + 2]
            while enemy_pieces:
                sq, enemy_pieces = pop_lsb(enemy_pieces)
                if bishop_attacks(sq, occ) & king_zone:
                    units += BISHOP_ATTACK_UNITS
            enemy_pieces = boards[enemy + 3]
            while enemy_pieces:
                sq, enemy_pieces = pop_lsb(enemy_pieces)
                if rook_attacks(sq, occ) & king_zone:
                    units += ROOK_ATTACK_UNITS
            enemy_pieces = boards[enemy + 4]
            while enemy_pieces:
                sq, enemy_pieces = pop_lsb(enemy_pieces)
                if (bishop_attacks(sq, occ) | rook_attacks(sq, occ)) & king_zone:
                    units += QUEEN_ATTACK_UNITS
            if units > KING_DANGER_MAX_UNITS:
                units = KING_DANGER_MAX_UNITS
            mg_score -= sign * KING_DANGER_MG[units]
