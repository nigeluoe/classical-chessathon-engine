...
than the added accuracy bought back in this pure-Python engine at a fast time control.
This candidate keeps V17's evaluator and adds a narrowly bounded search-quality experiment:
one extra ply after a checking move at shallow depth. The extension is limited to one per line,
so it can verify forcing checks and sacrifices without allowing checking sequences to grow the
