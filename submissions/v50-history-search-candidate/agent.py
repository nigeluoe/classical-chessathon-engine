"""V50: history-guided classical PVS chess engine.
...
def move_order(
    ...
    for index in range(count):
        ...
        if move == tt_move:
            ...
            # Search losing captures after quiets, but retain promotions and
            # cheap equal/up trades without paying for an exchange analysis.
            if (not promo and not _ep and PIECE_VALUE[piece % 6] > PIECE_VALUE[captured % 6]
                    and c.see_capture(boards, frm, to, piece, captured) < 0):
                score -= 5_000_000
        elif promo:
...
def search_node(
    ...
    move_order(boards, moves, scores, count, tt_move, killers, history, meta[0], ply)
    ...
    for index in range(count):
        ...
        if (depth >= 3 and legal_count >= 4 and quiet and not in_check and not gives_check
            ...
            if not pv_node and depth >= 9 and legal_count >= 12:
                reduction += 1
            # History is bounded and now includes failed quiets, so its sign
            # distinguishes useful moves from merely frequently tried moves.
            if history[side, frm, to] > 4_000:
                reduction -= 1
            elif history[side, frm, to] < -4_000 and not pv_node:
                reduction += 1
        ...
        if score > alpha:
            ...
            if alpha >= beta:
                if quiet:
                    ...
                    # Earlier legal quiets failed to produce this cutoff.
                    # Pruned/illegal entries are marked below and skipped.
                    for previous in range(index):
                        if scores[previous] == -INF * INF:
                            continue
                        prior = int(moves[previous])
                        pf, pt, _p, pc, pp, _e, _d, _k, _q = c.unpack_move(prior)
                        if pc == c.NO_PIECE and pp == 0:
                            old = int(history[side, pf, pt])
                            history[side, pf, pt] = old - bonus - old * bonus // HISTORY_LIMIT
                break
    ...
    if len(legal) > 1 and hard >= 0.004:
        ...
        if move is not None:
            ...
            if candidate in legal:
                chosen = candidate
    print(f"v50 depth={depth} nodes={nodes} score={score} move={chosen.uci()}", flush=True)
    board.push(chosen)
