...
addition: a rook-on-open-file bonus. A rook with no pawns in front of it on its own file
(semi-open) or no pawns of either colour on that file (fully open) gets a flat bonus, applied
equally to the midgame and endgame totals since rook activity on an open file matters
throughout the game, not just in the middlegame.

Built fresh from V11, not stacked on v14 (pawn structure, a real regression at 22.5% versus
V11) or v15 (king safety, a wash at 45.0% versus V11) -- see training/data/v14_vs_v11_20games
.log and training/data/v15_vs_v11_20games.log. Both of those failures came from evaluate()
getting slower without a big enough accuracy payoff to afford the lost search depth, so this
candidate is built to be as cheap as the cheapest of the two: pawn-file occupancy is computed
once per side (a single pass over at most 16 pawns total), then reused for at most 4 rooks, no
move generation. Gated on a real speed measurement before any game was played, same as every
other candidate this round.
"""
...
# Rook-on-open-file bonuses. Flat (not tapered): useful in both the middlegame and endgame.
...
def rook_open_file_score(
    ...
    either colour (open) in front of it on its own file. Reuses pawn-file occupancy computed
    once in evaluate() so no extra pawn scan happens per rook.
    """
...
def evaluate(board: chess.Board) -> int:
    ...
    A flat rook-on-open-file bonus is added to both totals equally, since it matters in both
    phases. The blended result is then interpolated by a material-based phase estimate.
    """
    mg_white = mg_black = 0
    eg_white = eg_black = 0
    phase = 0
    ...
    for square, piece in board.piece_map().items():
        ...
        if piece_type == chess.PAWN:
            if piece.color == chess.WHITE:
                white_pawn_files[chess.square_file(square)] = True
            else:
                black_pawn_files[chess.square_file(square)] = True

    rook_white = rook_open_file_score(board, chess.WHITE, white_pawn_files, black_pawn_files)
    rook_black = rook_open_file_score(board, chess.BLACK, white_pawn_files, black_pawn_files)
    mg_white += rook_white
    eg_white += rook_white
    mg_black += rook_black
    eg_black += rook_black
