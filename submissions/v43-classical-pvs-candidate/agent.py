"""V43: independently written classical PVS chess engine.
    ...
    if len(legal) > 1 and hard >= 0.004:
        ...
        if move is not None:
            ...
            if candidate in legal:
                chosen = candidate
    print(f"v43 depth={depth} nodes={nodes} score={score} move={chosen.uci()}", flush=True)
    board.push(chosen)
...
_warm_result = _ENGINE.search(_warm_boards, _warm_meta, 0.2, 12.0, max_depth=5)
