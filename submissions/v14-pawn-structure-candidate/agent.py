...
addition: a pawn-structure term. Doubled, isolated, and passed pawns are scored and blended
into the same midgame/endgame taper as the rest of the evaluator, so a passed pawn is worth
much more once material has been traded off than it is in a crowded middlegame. Unlike the
mobility experiment (v13, rejected: a ~4x evaluate() slowdown from extra legal-move
generation cost far more search depth than it bought back), pawn-structure checks only walk
each side's own pawns (at most 8) against the opponent's pawns (at most 8) using plain square
arithmetic, so no extra move generation is needed and the cost should be small. Measured, not
assumed: see the microbenchmark this candidate was gated on before any game was played.
"""
...
# Pawn-structure weights, midgame and endgame. A passed pawn matters far more once the board
# has emptied out, so its bonus grows steeply with rank and is much larger in the endgame.
DOUBLED_PAWN_MG = 10
DOUBLED_PAWN_EG = 20
ISOLATED_PAWN_MG = 12
ISOLATED_PAWN_EG = 18
...
PASSED_PAWN_EG = [0, 10, 20, 40, 70, 110, 170, 0]
...
def _is_passed(square: chess.Square, colour: chess.Color, enemy_pawns: chess.SquareSet) -> bool:
    """A pawn is passed if no enemy pawn on its file or an adjacent file can ever block or
    capture it on its way to promotion."""
    file = chess.square_file(square)
    rank = chess.square_rank(square)
    for enemy_square in enemy_pawns:
        if abs(chess.square_file(enemy_square) - file) > 1:
            continue
        enemy_rank = chess.square_rank(enemy_square)
        if colour == chess.WHITE and enemy_rank > rank:
            return False
        if colour == chess.BLACK and enemy_rank < rank:
            return False
    return True


def pawn_structure_score(board: chess.Board, colour: chess.Color) -> tuple[int, int]:
    """Return (midgame, endgame) doubled/isolated/passed pawn scores for one side.

    Only walks this side's own pawns (at most 8) against the opponent's pawns (at most 8),
    using plain square arithmetic -- no move generation, unlike the mobility experiment that
    this candidate deliberately avoids repeating.
    """
    ...
    for square in own_pawns:
        ...
        if left_count == 0 and right_count == 0:
            mg -= ISOLATED_PAWN_MG
            eg -= ISOLATED_PAWN_EG
        if _is_passed(square, colour, enemy_pawns):
            forward_rank = rank if colour == chess.WHITE else 7 - rank
            mg += PASSED_PAWN_MG[forward_rank]
            eg += PASSED_PAWN_EG[forward_rank]
    return mg, eg
...
def evaluate(board: chess.Board) -> int:
    ...
    doubled, isolated, and passed pawns add or subtract from those same two totals. The
    blended result is then interpolated by a material-based phase estimate so, for example,
    a passed pawn is worth much more once material has been traded off than it is in a
    crowded middlegame.
    """
    mg_white = mg_black = 0
    eg_white = eg_black = 0
    ...
    white_pawn_mg, white_pawn_eg = pawn_structure_score(board, chess.WHITE)
    black_pawn_mg, black_pawn_eg = pawn_structure_score(board, chess.BLACK)
