...
This is V6's search (PVS negamax, alpha-beta, transposition table, killer/history move
ordering, null-move pruning, LMR, futility pruning, aspiration windows, iterative deepening)
with one change: a tapered, PeSTO-style evaluator replaces V2/V6's hand-rolled centrality
proxy. Material and piece placement are each scored twice, once with midgame weights and once
with endgame weights, and interpolated by a material-based game-phase estimate, so a knight
posted deep in enemy territory is worth more in the middlegame than in a bare-king endgame and
a king is rewarded for shelter early and for centralising once the board empties.

It also carries forward a defensive fix from the v7 experiment: quiescence search restores the
board with try/finally, so a mid-search timeout cannot leave a pushed move behind. It does not
carry forward v7's root-level draw-decline logic: a real 20-game arena (recorded in
training/data/v7_vs_v6_20games.log) showed v7 scoring only 42.5% against this same v6 search,
i.e. a real regression, most likely because declining a repetition draw on a shallow, noisy
score estimate converts some safe draws into losses. v6, not v7, is therefore the actual
validated current-best and the base for this candidate.
"""
