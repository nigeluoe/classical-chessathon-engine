...
addition: root-level threefold-repetition awareness, carried over from the v7 experiment's
fixed draw policy. Decline a claimable repetition only when the search finds a clearly
winning score; otherwise take the draw. The fixed policy (`DRAW_DECLINE_SCORE = 150`,
"decline only when winning") is a real, measured non-regression against v6 (55% over 20
games, training/data/v7fixed_vs_v6_20games.log) rather than the original, more aggressive
"keep playing unless losing badly" policy that actually caused v7's earlier 42.5% regression
(training/data/v7_vs_v6_20games.log) -- the two policies are easy to conflate because they
lived under the same v7 label; see submissions/README.md for the timeline.

This candidate exists to answer one question: does draw-awareness add anything on top of the
tapered evaluator, or is it neutral/negative stacked with a stronger eval? Measure, don't
assume, per the team's own standing rule.
"""
...
# The process is one game long, so this is a complete repetition history for that game.
...
def evaluate(board: chess.Board) -> int:
    ...
    the two totals are blended by a material-based phase estimate so, for example, a knight
    on an outpost scores higher in the middlegame than in a bare endgame, and the king is
    rewarded for staying sheltered early and for centralising once material has been traded
    off.
    """
    mg_white = mg_black = 0
    eg_white = eg_black = 0
...
def record_position(board: chess.Board) -> None:
    """Remember an observed or selected position for threefold-repetition decisions."""
    key = position_key(board)
    POSITION_COUNTS[key] = POSITION_COUNTS.get(key, 0) + 1


def is_third_repetition(board: chess.Board, move: chess.Move) -> bool:
    """Return whether this root move would create a claimable threefold repetition."""
    board.push(move)
    try:
        return POSITION_COUNTS.get(position_key(board), 0) >= 2
    finally:
        board.pop()


def should_claim_draw(draw_available: bool, best_score: int) -> bool:
    """Decline repetition only when search finds a clear enough winning advantage."""
    return draw_available and best_score <= DRAW_DECLINE_SCORE
...
def search_root(
    ...
    draw_move: chess.Move | None = None
    for move_index, move in enumerate(moves):
        if is_third_repetition(board, move):
            draw_move = move
            continue
        board.push(move)
        try:
        ...
        if score > best_score:
            ...
            best_move = move
        alpha = max(alpha, score)
    if draw_move is not None and should_claim_draw(True, best_score):
        # Preserve a draw only when normal play is clearly losing or unavailable.
        return draw_move, 0
...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    if preferred not in current_legal_moves:
        # An interrupted aspiration re-search must never leak a stale root move.
        preferred = current_legal_moves[0]
