...
computed inside the same jitted pass) with one narrowly bounded search-quality experiment
added on top: one extra ply after a checking move at shallow depth. The extension is limited
to one per line (a fixed budget threaded through the search, not a per-node reset), so it can
verify forcing checks and sacrifices without allowing checking sequences to grow the search
without bound. This idea (V21) was previously built on V17 but never benchmarked; since V20
superseded V17 as the leading candidate before that benchmark ran, this candidate re-bases the
same idea on V20 instead of measuring it on the now-superseded evaluator.
...
function, and V15's king-safety function exactly as this candidate is supposed to. That check
is unaffected by the check-extension change: it exercises `evaluate()` only, not `negamax()`.
"""
...
FUTILITY_MARGIN = 110
# A single shallow check extension improves verification of forcing checks without unbounded
# extension chains. It is intentionally a search-quality experiment, not an evaluation change.
CHECK_EXTENSION_MAX_DEPTH = 3
CHECK_EXTENSION_BUDGET = 1
...
def negamax(
    board: chess.Board, depth: int, alpha: int, beta: int, ply: int, check_extensions_left: int
) -> int:
    ...
    if (
        ...
        try:
            null_score = -negamax(
                board, depth - 3, -beta, -beta + 1, ply + 1, check_extensions_left
            )
        finally:
            board.pop()
        if null_score >= beta:
    ...
    for move_index, move in enumerate(moves):
        ...
        extension = int(
            gives_check
            and depth <= CHECK_EXTENSION_MAX_DEPTH
            and check_extensions_left > 0
        )
        child_extensions_left = check_extensions_left - extension
        board.push(move)
        try:
            if move_index == 0:
                score = -negamax(
...
def search_root(
    ...
    for move_index, move in enumerate(moves):
        ...
        try:
            if move_index == 0:
                score = -negamax(
                    board, depth - 1, -beta, -alpha, 1, CHECK_EXTENSION_BUDGET
                )
            else:
                score = -negamax(
                    board, depth - 1, -alpha - 1, -alpha, 1, CHECK_EXTENSION_BUDGET
                )
                if alpha < score < beta:
                    score = -negamax(
                        board, depth - 1, -beta, -alpha, 1, CHECK_EXTENSION_BUDGET
