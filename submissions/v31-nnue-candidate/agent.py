...
# --------------------------------------------------------------------------------------------
# NNUE-lite: a small, incrementally-updatable learned evaluator threaded through the same
# all-numba search below, gated on the material+PST+pawn+king evaluator above until it clears
# real correctness, speed, and strength bars (this file's docstring order of work). Two
# 128-value accumulators (one per perspective -- "as seen by White", "as seen by Black") are
# maintained incrementally inside apply_move_nnue/unmake_move_nnue, paired 1:1 with every
# apply_move_hashed/unmake_move_hashed call the search already makes. This is deliberately
# unlike V4 (`submissions/v4-incremental-candidate/`), whose accumulator math was correct but
# was wrapped in Python-object state copying that ate the entire speed benefit (TEAM_GUIDE.md's
# V4 row): here the accumulator is a plain float32[2, ACCUMULATOR_SIZE] numpy array living
# alongside `boards`/`meta`/`hash_arr` in the same nopython call chain, never a Python object.
#
# Feature set (adapted from training/v10_nnue.py's king-relative design; kept smaller per this
# round's task -- no material-phase embedding, so the accumulator table is exactly V10's own
# size, 32 mirrored king buckets x 10 own/enemy piece categories x 64 squares = 20,480 rows,
# but the head is smaller, 256 -> 32 -> 16 -> 1 with no extra phase input):
#
#   - Kings are never features. This is what makes the incremental update cheap: a king move
#     changes no OTHER piece's feature index, so it only ever requires refreshing the mover's
#     OWN perspective accumulator from scratch (`refresh_accumulator`, one pass over the <=30
#     non-king pieces on the board) -- the opponent's perspective accumulator needs no change
#     at all for a plain king move. A castle is the one case where the non-moving perspective
#     still needs an update: the rook (a real feature) moved too, so it gets one incremental
#     add/remove on the perspective whose king did NOT just move.
#   - Every other move (the overwhelming majority of plies) touches at most 2-4 feature rows
#     per perspective (the mover's old/new square, an optional captured piece, an optional
#     castling rook) and is updated with a handful of length-128 vector add/subtracts -- no
#     recomputation of anything from the board.
#   - A null move (`apply_null_move`/`unmake_null_move`) moves no piece, so it needs no
#     accumulator change at all -- only `meta[0]` flips, and `nnue_eval` already reads
#     `acc[side_to_move]` as "us", so the flip alone is enough.
#
# Correctness gate (`training/test_v31_nnue_incremental.py`): the incrementally-updated
# accumulator is checked against `compute_accumulators` (a full from-scratch recompute) after
# every move AND after every unmake, across thousands of plies of self-play, with an untrained
# random-weight network, before any training happens -- same discipline as V4's 7,975-check
# precedent and this repo's own bar for this kind of change.
# --------------------------------------------------------------------------------------------

KING_BUCKETS = 16
PIECE_CATEGORIES = 10  # own/enemy x (pawn, knight, bishop, rook, queen); kings excluded.
NNUE_SQUARES = 64
NNUE_FEATURE_COUNT = KING_BUCKETS * PIECE_CATEGORIES * NNUE_SQUARES  # 10,240
ACCUMULATOR_SIZE = 64
NNUE_HIDDEN1 = 32
NNUE_HIDDEN2 = 16
# --------------------------------------------------------------------------------------------
# V31 round 2: cut both dimensions of the table this round's coordinator review flagged as the
# real speed cost (measured: apply_move_nnue/unmake_move_nnue ~2.3x costlier per move than
# V29's plain apply_move_hashed/unmake_move_hashed, traced to this table's cache-unfriendly,
# king-position-dependent access pattern -- not the network's forward pass, which was already
# cheaper per call than V29's material+PST evaluator). King buckets 32 -> 16 (pair adjacent
# ranks instead of the original one-bucket-per-rank scheme, still keeping file-mirror symmetry
# and a coarse back-rank/advanced distinction) and the accumulator 128 -> 64 together cut the
# feature-transformer table from 20,480 x 128 (10.5 MB float32) to 10,240 x 64 (2.6 MB), a real
# 4x reduction in the table's cache footprint, plus half the float32 add/subtract work per
# feature touched. This is a deliberate, this-round's-own instance of the repo's established
# lesson (V13-V19, and now this): a smaller-but-fast representation beats a richer-but-slow one
# at these time controls -- re-measured, not assumed, in this round's own re-run of the speed
# gate. This does NOT reuse `training/v10_nnue.py`'s feature encoder any more (that file is a
# fixed-size, 32-bucket experiment of its own and was not touched): `training/train_v31_nnue.py`
# now carries its own encoder matching these constants exactly, verified against agent.py's own
# `feature_row`/`king_bucket_and_mirror` the same way the original 32-bucket version was.
# --------------------------------------------------------------------------------------------


@njit(cache=False)
def king_bucket_and_mirror(king_sq: int, persp_is_black: bool) -> tuple[int, bool]:
    """16-way mirrored king bucket: fold the king onto files e-h (a king on c1 and one on f1
    share a bucket) AND pair adjacent ranks (ranks 0-1 share a bucket, 2-3, 4-5, 6-7), so the
    king bucket still separates back-rank/advanced and kingside/queenside placement but at
    half the granularity of the original 32-way scheme -- see the constants above for why.
    Split out from the per-piece feature lookup (`feature_row` below) specifically so a caller
    touching several pieces for the same king (`refresh_accumulator`'s up-to-30-piece loop;
    `update_accumulator_incremental`'s 2-4 feature lookups per move) computes this once and
    reuses it, rather than re-deriving the same bucket from scratch on every piece -- a real
    measured cost, not just a style choice (an earlier version folded this into one function
    per piece and paid for it in accumulator update speed)."""
    mirror_files = (king_sq % 8) < 4
    kf = king_sq % 8
    kr = king_sq // 8
    if persp_is_black:
        kr = 7 - kr
    if mirror_files:
        kf = 7 - kf
    return (kr // 2) * 4 + kf - 4, mirror_files


@njit(cache=False)
def feature_row(
    bucket: int, mirror_files: bool, persp_is_black: bool, piece_code: int, sq: int,
) -> int:
    """Feature row for one non-king piece as seen by one perspective, given that perspective's
    already-computed king bucket/mirror (see `king_bucket_and_mirror`). `piece_code` must
    never be WK/BK (kings are not features -- see the NNUE section's module docstring)."""
    sf = sq % 8
    sr = sq // 8
    if persp_is_black:
        sr = 7 - sr
    if mirror_files:
        sf = 7 - sf
    square_index = sr * 8 + sf

    piece_is_white = piece_code < 6
    piece_type0 = piece_code % 6  # 0..4 = pawn..queen
    own = piece_is_white != persp_is_black
    category = piece_type0 if own else piece_type0 + 5
    return (bucket * PIECE_CATEGORIES + category) * NNUE_SQUARES + square_index


@njit(cache=False)
def refresh_accumulator(
    boards: np.ndarray, king_sq: int, persp_is_black: bool, acc_row: np.ndarray,
) -> None:
    """Recompute one perspective's accumulator from scratch -- the correctness oracle this
    module's own incremental path is checked against, and also the real code path used
    whenever that perspective's own king just moved. Reads the module-level NNUE_W1/NNUE_B1
    globals (see the note above `NNUE_W1, NNUE_B1, ... = load_nnue_weights()`)."""
    acc_row[:] = NNUE_B1
    bucket, mirror_files = king_bucket_and_mirror(king_sq, persp_is_black)
    for code in range(12):
        if code in (WK, BK):
            continue
        bb = boards[code]
        while bb:
            s, bb = pop_lsb(bb)
            acc_row += NNUE_W1[feature_row(bucket, mirror_files, persp_is_black, code, s)]


@njit(cache=False)
def compute_accumulators(boards: np.ndarray) -> np.ndarray:
    """Full from-scratch (2, ACCUMULATOR_SIZE) accumulator pair for a position -- used to seed
    the root accumulator at the start of a search, and as the reference in the correctness
    test."""
    acc = np.empty((2, NNUE_W1.shape[1]), dtype=np.float32)
    refresh_accumulator(boards, king_square(boards, True), False, acc[0])
    refresh_accumulator(boards, king_square(boards, False), True, acc[1])
    return acc


@njit(cache=False)
def update_accumulator_incremental(
    acc_row: np.ndarray, king_sq: int, persp_is_black: bool, piece: int, add_piece: int,
    frm: int, to: int, captured: int, capture_sq: int, ck: bool, cq: bool,
    mover_is_white: bool,
) -> None:
    """O(1) update for a perspective whose own king did NOT move this ply: remove the mover's
    old-square feature, add its new-square feature (the promoted piece's code, if this was a
    promotion), remove a captured piece's feature if any, and -- only possible together with
    the OTHER perspective's king move, since castling only co-occurs with a king move -- update
    the castling rook's feature too, since the rook is a real feature piece even though the
    king that moved alongside it is not.

    `piece`/`add_piece` can themselves be WK/BK here: this branch runs for the perspective
    whose OWN king did not move, but `piece` is whatever piece actually moved this ply, which
    can be the OTHER side's king (an ordinary king walk, or the king move that drives a
    castle). Kings are never features (see the NNUE section's module docstring), so that case
    must skip the piece-square feature update entirely -- only a captured piece or a castling
    rook (both real features) still need one.

    The castling-rook squares are computed once via `mover_is_white`'s own arithmetic rather
    than as two separate white/black branches -- the same "unify duplicate branches into one
    shared code path" technique V26's own module docstring credits for a real, measured
    compile-time win (see the constants above this function)."""
    bucket, mirror_files = king_bucket_and_mirror(king_sq, persp_is_black)
    if piece != WK and piece != BK:
        acc_row -= NNUE_W1[feature_row(bucket, mirror_files, persp_is_black, piece, frm)]
        acc_row += NNUE_W1[feature_row(bucket, mirror_files, persp_is_black, add_piece, to)]
    if captured != NO_PIECE:
        acc_row -= NNUE_W1[feature_row(bucket, mirror_files, persp_is_black, captured, capture_sq)]
    if ck or cq:
        rook_code = WR if mover_is_white else BR
        rank_base = 0 if mover_is_white else 56
        rook_from = rank_base + 7 if ck else rank_base
        rook_to = rank_base + 5 if ck else rank_base + 3
        acc_row -= NNUE_W1[feature_row(bucket, mirror_files, persp_is_black, rook_code, rook_from)]
        acc_row += NNUE_W1[feature_row(bucket, mirror_files, persp_is_black, rook_code, rook_to)]


@njit(cache=False)
def update_one_perspective(
    boards: np.ndarray, acc_row: np.ndarray, own_king_moved: bool, king_sq: int,
    persp_is_black: bool, piece: int, add_piece: int, frm: int, to: int, captured: int,
    capture_sq: int, ck: bool, cq: bool, mover_is_white: bool,
) -> None:
    """Update one perspective's accumulator for one move: a full refresh if that perspective's
    own king just moved, otherwise the O(1) incremental path. Split out of `apply_move_nnue`
    (called twice, once per perspective, instead of two separate `if piece == WK: ... else:
    ...` / `if piece == BK: ... else: ...` blocks inline) for the same reason as
    `update_accumulator_incremental`'s castling unification above -- one shared branch instead
    of two near-identical ones, and a smaller `apply_move_nnue` body for numba to compile."""
    if own_king_moved:
        refresh_accumulator(boards, king_sq, persp_is_black, acc_row)
    else:
        update_accumulator_incremental(
            acc_row, king_sq, persp_is_black, piece, add_piece, frm, to, captured, capture_sq,
            ck, cq, mover_is_white,
        )
...
def apply_move_nnue(
    boards: np.ndarray, meta: np.ndarray, m: int, undo: np.ndarray, hash_arr: np.ndarray,
    acc: np.ndarray, undo_acc: np.ndarray,
) -> None:
    """Apply a move to boards/meta/hash (via apply_move_hashed) AND update both NNUE
    perspective accumulators to match, snapshotting the pre-move values into undo_acc so
    unmake_move_nnue can restore them exactly. Paired 1:1 with apply_move_hashed at every
    search call site, the same way apply_move_hashed itself pairs with apply_move."""
    undo_acc[0, :] = acc[0]
    undo_acc[1, :] = acc[1]
    frm, to, piece, captured, promo, ep, _dbl, ck, cq = unpack_move(m)
    mover_is_white = piece < 6
    apply_move_hashed(boards, meta, m, undo, hash_arr)
    add_piece = promo_piece_code(promo, mover_is_white) if promo != PROMO_NONE else piece
    capture_sq = (to - 8 if mover_is_white else to + 8) if ep else to
    update_one_perspective(
        boards, acc[0], piece == WK, king_square(boards, True), False, piece, add_piece, frm,
        to, captured, capture_sq, ck, cq, mover_is_white,
    )
    update_one_perspective(
        boards, acc[1], piece == BK, king_square(boards, False), True, piece, add_piece, frm,
        to, captured, capture_sq, ck, cq, mover_is_white,
    )


@njit(cache=False)
def unmake_move_nnue(
    boards: np.ndarray, meta: np.ndarray, undo: np.ndarray, hash_arr: np.ndarray,
    acc: np.ndarray, undo_acc: np.ndarray,
) -> None:
...
@njit(cache=False)
def linear_relu(w: np.ndarray, x: np.ndarray, b: np.ndarray, out: np.ndarray) -> None:
    """out = relu(w @ x + b), written as plain loops: numba's nopython np.dot for a
    matrix-vector product needs scipy for its BLAS binding, and scipy is not in the
    platform's fixed package set (AGENTS.md), so this hand-rolled reduction is the portable
    version -- these layers are tiny (<=32 wide) so the loop cost is negligible."""
    rows, cols = w.shape
    for r in range(rows):
        total = b[r]
        row = w[r]
        for c in range(cols):
            total += row[c] * x[c]
        out[r] = total if total > 0.0 else 0.0


@njit(cache=False)
def nnue_eval(acc: np.ndarray, side_to_move: int) -> int:
    """Evaluate from the side to move's perspective, same convention as evaluate() above.
    Reads the module-level NNUE_W2/NNUE_B2/NNUE_W3/NNUE_B3/NNUE_W4/NNUE_B4 globals."""
    h = acc.shape[1]
    x = np.empty(2 * h, dtype=np.float32)
    us = acc[side_to_move]
    them = acc[1 - side_to_move]
    for i in range(h):
        u = us[i]
        x[i] = 0.0 if u < 0.0 else (1.0 if u > 1.0 else u)
        t = them[i]
        x[h + i] = 0.0 if t < 0.0 else (1.0 if t > 1.0 else t)
    hidden1 = np.empty(NNUE_W2.shape[0], dtype=np.float32)
    linear_relu(NNUE_W2, x, NNUE_B2, hidden1)
    hidden2 = np.empty(NNUE_W3.shape[0], dtype=np.float32)
    linear_relu(NNUE_W3, hidden1, NNUE_B3, hidden2)
    total = NNUE_B4[0]
    w4_row = NNUE_W4[0]
    for c in range(NNUE_W4.shape[1]):
        total += w4_row[c] * hidden2[c]
    total *= NNUE_CENTIPAWN_SCALE
    # `total` is float32 inside this @njit function, not a Python float -- numba's round() on
    # a float does not return a Python int the way CPython's does, so the explicit int() cast
    # is load-bearing here despite ruff's plain-Python-semantics suggestion otherwise.
    return int(round(total))  # noqa: RUF046
...
def make_random_nnue_weights(seed: int = 20260908) -> NnueWeights:
    """Untrained, fixed-seed random weights -- used for the correctness and speed gates before
    any training happens (this round's own required order of work: prove the runtime shape is
    correct and fast before training quality matters at all). `b1` is initialised to 0.5, the
    midpoint of the [0, 1] clipped-ReLU range, so an empty/near-empty accumulator sum still
    lands inside the clip instead of always saturating at 0. The 100.0 scale matches the real
    trained-weights convention below (`centipawn_scale` in the exported .npz), so a speed
    benchmark run against these random weights sees the same output-magnitude regime a trained
    network will actually produce, not an artificially tiny or huge one."""
    rng = np.random.default_rng(seed)
    w1 = rng.normal(0.0, 0.05, size=(NNUE_FEATURE_COUNT, ACCUMULATOR_SIZE)).astype(np.float32)
    b1 = np.full(ACCUMULATOR_SIZE, 0.5, dtype=np.float32)
    w2 = rng.normal(0.0, 0.1, size=(NNUE_HIDDEN1, ACCUMULATOR_SIZE * 2)).astype(np.float32)
    b2 = np.zeros(NNUE_HIDDEN1, dtype=np.float32)
    w3 = rng.normal(0.0, 0.1, size=(NNUE_HIDDEN2, NNUE_HIDDEN1)).astype(np.float32)
    b3 = np.zeros(NNUE_HIDDEN2, dtype=np.float32)
    w4 = rng.normal(0.0, 0.1, size=(1, NNUE_HIDDEN2)).astype(np.float32)
    b4 = np.zeros(1, dtype=np.float32)
    return w1, b1, w2, b2, w3, b3, w4, b4, 100.0


def load_nnue_weights() -> NnueWeights:
    """Load trained weights from weights/nnue_v1.npz next to this file, if present; otherwise
    fall back to fixed-seed random weights (the correctness/speed-gate configuration -- see
    the module docstring). Stockfish and every offline training tool never run at competition
    time; the weights file, if present, ships inside the submission zip via
    harness/package.py's DEFAULT_INCLUDES = ("weights",).

    `centipawn_scale` (default 100.0 if the file predates this field) undoes
    `training/train_v31_nnue.py`'s `labels / 100.0` training-time normalisation (a standard,
    training-stability-only rescaling -- the network trains on "pawns", not centipawns, then
    this multiplies its raw output back to the centipawn units the rest of the search uses).
    A real bug caught by `training/test_v31_nnue_weights_parity.py`: without this, the first
    trained weights export evaluated exactly 100x too small everywhere.

    Shape-checked against the current `NNUE_FEATURE_COUNT`/`ACCUMULATOR_SIZE` before use and
    silently discarded (falling back to random weights) on any mismatch: a real risk, not a
    hypothetical one -- this round's own feature-space reduction (see the constants above)
    made an earlier exported file's shape stale, and a stale file loaded without a shape check
    would not crash, it would silently index into the wrong rows and evaluate garbage."""
    weights_path = _Path(__file__).resolve().parent / "weights" / "nnue_v1.npz"
    if weights_path.is_file():
        data = np.load(weights_path)
        w1 = np.ascontiguousarray(data["w1"], dtype=np.float32)
        if w1.shape == (NNUE_FEATURE_COUNT, ACCUMULATOR_SIZE):
            scale = float(data["centipawn_scale"][0]) if "centipawn_scale" in data else 100.0
            return (
                w1,
                np.ascontiguousarray(data["b1"], dtype=np.float32),
                np.ascontiguousarray(data["w2"], dtype=np.float32),
                np.ascontiguousarray(data["b2"], dtype=np.float32),
                np.ascontiguousarray(data["w3"], dtype=np.float32),
                np.ascontiguousarray(data["b3"], dtype=np.float32),
                np.ascontiguousarray(data["w4"], dtype=np.float32),
                np.ascontiguousarray(data["b4"], dtype=np.float32),
                scale,
            )
    return make_random_nnue_weights()


# Loaded once at import, before any jitted function below is warmed up, and never reassigned
# afterward -- the same "compute once, read as a plain module-level global inside every @njit
# function that needs it" pattern this file already uses for ZOBRIST_PIECE/KNIGHT_ATTACKS/the
# magic-bitboard tables (see e.g. `compute_hash`, `apply_move_hashed`, `square_attacked`).
# Threading these through negamax/quiescence/search_root/try_null_move as extra parameters
# (an earlier version of this file did exactly that, with a bundled `nn: tuple` to keep the
# signatures shorter) measurably slowed numba's compile time for those already-large recursive
# functions with no runtime benefit -- these arrays are fixed for the whole process, exactly
# like the Zobrist tables, so they belong at module scope, not on the call stack.
(
    NNUE_W1, NNUE_B1, NNUE_W2, NNUE_B2, NNUE_W3, NNUE_B3, NNUE_W4, NNUE_B4,
    NNUE_CENTIPAWN_SCALE,
) = load_nnue_weights()
...
def quiescence(
    ...
    if qdepth >= MAX_QUIESCENCE_PLY or ply >= MAX_PLY:
        return nnue_eval(acc, meta[0])

    in_check = is_in_check(boards, meta)
    stand_pat = nnue_eval(acc, meta[0])
    if not in_check:
        if stand_pat >= beta:
            return beta
        if stand_pat > alpha:
    ...
    undo = np.empty(6, dtype=np.int64)
    undo_acc = np.empty((2, ACCUMULATOR_SIZE), dtype=np.float32)
    for i in range(count):
        apply_move_nnue(boards, meta, buf[i], undo, hash_arr, acc, undo_acc)
        score = -quiescence(
            boards, meta, hash_arr, -beta, -alpha, ply + 1, qdepth + 1,
            node_count, deadline, timed_out, acc,
        )
        unmake_move_nnue(boards, meta, undo, hash_arr, acc, undo_acc)
        if timed_out[0]:
            return 0
        if score >= beta:
            return beta
...
def try_null_move(
    ...
    tt_key: np.ndarray, tt_depth: np.ndarray, tt_score: np.ndarray, tt_flag: np.ndarray,
    tt_move_arr: np.ndarray, killers: np.ndarray, history: np.ndarray, acc: np.ndarray,
) -> tuple[bool, int]:
    ...
    has_non_pawn_material-gated null move. Returns (cutoff, score). A null move touches no
    piece, so `acc` needs no update at all -- only `meta[0]` flips (see the NNUE section's
...
def negamax(
    ...
    null_cutoff, null_score = try_null_move(
        ...
        tt_key, tt_depth, tt_score, tt_flag, tt_move_arr, killers, history, acc,
    )
    if timed_out[0]:
        return 0
    if null_cutoff:
    ...
    static_eval = nnue_eval(acc, side) if depth == 1 else -MATE_SCORE

    undo = np.empty(6, dtype=np.int64)
    undo_acc = np.empty((2, ACCUMULATOR_SIZE), dtype=np.float32)
    searched_quiet = np.empty(MAX_MOVES, dtype=np.int64)
    quiet_count = 0
    ...
    for i in range(count):
        ...
        if i == 0:
            score = -negamax(
                ...
                tt_move_arr, killers, history, acc,
            )
        else:
...
def search_root(
    ...
    killers: np.ndarray, history: np.ndarray, acc: np.ndarray,
) -> tuple[int, int]:
    ...
    for i in range(root_count):
        apply_move_nnue(boards, meta, root_moves[i], undo, hash_arr, acc, undo_acc)
        if i == 0:
            score = -negamax(
...
class Engine:
    ...
    def __init__(self) -> None:
        ...
        # NNUE weights are module-level globals (NNUE_W1, ... -- see the note above their
        # assignment), loaded once at import; nothing per-instance to set up here.
    ...
    def search(
        ...
        acc = compute_accumulators(boards)
        ...
        for depth in range(1, max_depth + 1):
            ...
            if depth == 1:
                ...
                move, score = search_root(
                    ...
                    root_copy, root_count, tt_best, self.killers, self.history, acc,
                )
            else:
