...
PONDER_THREAD: threading.Thread | None = None
DEADLINE = 0.0
NODES = 0
...
def check_time() -> None:
    ...
    global NODES
    NODES += 1
    if PONDER_THREAD is threading.current_thread():
        if NODES % TIME_CHECK_INTERVAL == 0 and PONDER_STOP.is_set():
            raise SearchTimeout
        return
...
def stop_ponder() -> None:
    """Finish background work before a new real move search uses shared state."""
...
def ponder(board: chess.Board) -> None:
    """Use opponent time to prefill the table for their most plausible replies."""
    try:
        for depth in range(2, PONDER_MAX_DEPTH + 1):
            entry = TRANSPOSITION_TABLE.get(position_key(board))
            moves = ordered_moves(board, 0, entry.best_move if entry else None)
            for move in moves[:PONDER_ROOT_MOVES]:
                if PONDER_STOP.is_set():
                    return
                board.push(move)
                try:
                    negamax(board, depth, -MATE_SCORE, MATE_SCORE, 1)
                finally:
                    board.pop()
...
def start_ponder(board: chess.Board) -> None:
    """Start one daemon worker after returning our move to the opponent."""
    global PONDER_THREAD
    PONDER_STOP.clear()
    PONDER_THREAD = threading.Thread(target=ponder, args=(board,), daemon=True)
    PONDER_THREAD.start()
...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    ponder_board.push(preferred)
    start_ponder(ponder_board)
